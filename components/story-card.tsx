...
export function StoryCard({ story, currentUser, justPosted = false, onRequireIdentity, onDeleted, onLikeChanged }: StoryCardProps) {
  ...
  const [likeActive, setLikeActive] = useState(() => {
    ...
    try {
      const likedMap = JSON.parse(localStorage.getItem("weiji-liked-posts-v1") || "{}") as Record<string, number>;
      return Boolean(likedMap[noteId]);
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      communityService.getLikeState(noteId, currentUser.id, story.likes).then((state) => {
        if (!active) return;
        setLikeActive((current) => state.liked || current);
